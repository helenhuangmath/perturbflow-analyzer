...
[`perturbscope/pipeline.py`](perturbscope/pipeline.py); each step lives in
its own module under `perturbscope/`.
...
**Module:** [`perturbscope/genenet.py`](perturbscope/genenet.py)
...
colour everywhere) ([`genenet.py:548-554`](perturbscope/genenet.py#L548-L554)):
...
then build an undirected `networkx.Graph` ([`genenet.py:104-121`](perturbscope/genenet.py#L104-L121)):
...
([`genenet.py:325-329`](perturbscope/genenet.py#L325-L329)):
...
([`genenet.py:341-355`](perturbscope/genenet.py#L341-L355)):
...
The interactive report ([`perturbscope/interactive.py`](perturbscope/interactive.py)) base64-embeds the three per-perturbation networks side-by-side in row 1 of the *Gene Network* tab, and the `control_heatmap` + `<p>_heatmap_comparison` heatmaps in row 2.
...
* **TF–TF regulatory view.** The
  [`perturbscope/regulatory.py`](perturbscope/regulatory.py) module
